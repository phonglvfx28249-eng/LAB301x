import React, { useState } from "react";
import { Wallet, Lock, ChevronDown, Filter, ArrowLeft, X } from "lucide-react";
import { Link } from "react-router-dom";
import WindyLogo from "../components/common/windy-logo.jsx";
import { useUserResources } from "../context/UserResourcesContext.jsx";


function Dropdown({ label, value }) {
    return (
        <button className="flex items-center justify-between gap-3 px-3 py-2 rounded-md border border-gray-700/50 text-sm text-gray-900 min-w-[140px]">
            <span className="text-gray-500">{label}</span>
            <span className="flex items-center gap-1 font-semibold">
        {value}
                <ChevronDown size={14} strokeWidth={2} />
      </span>
        </button>
    );
}

function OrderDetail({ trade, onClose }) {
    if (!trade) return null;

    const rows = [
        { label: "Order id:", value: "Wind Lê" },
        {
            label: "Status:",
            value: (
                <span className="flex items-center gap-2 text-gray-900">
          <span className={`w-2.5 h-2.5 rounded-full ${trade.statusColor}`} />
                    {trade.status}
        </span>
            ),
        },
        { label: "Amount:", value: trade.amount },
        {
            label: "Coin:",
            value: (
                <span className="flex items-center gap-2 text-gray-900">
          <span className="font-extrabold tracking-tight text-emerald-600 text-stroke-1 text-stroke-color-black text-base">
            {trade.symbol}
          </span>
          ({trade.coin})
        </span>
            ),
        },
        { label: "Time:", value: trade.time },
        { label: "Price:", value: trade.price },
        {
            label: "ROI:",
            value: <span className="text-green-600 font-medium">{trade.roi}</span>,
        },
        {
            label: "Earned:",
            value: (
                <span className="text-green-600 font-medium">{trade.earned}</span>
            ),
        },
    ];

    return (
        <div className="w-full max-w-sm rounded-lg border border-gray-700/50 bg-[#EAE3D5] px-6 py-5 relative">
            <button
                onClick={onClose}
                aria-label="Close order detail"
                className="absolute top-4 right-4 text-red-500 hover:text-red-600 font-medium text-sm"
            >
                X
            </button>

            <h2 className="font-serif text-xl font-bold text-gray-900 mb-4">
                Order Detail
            </h2>

            <dl className="space-y-3.5 text-sm">
                {rows.map((row, i) => (
                    <div key={i} className="flex items-center">
                        <dt className="w-24 text-gray-500 shrink-0">{row.label}</dt>
                        <dd className="text-gray-900">{row.value}</dd>
                    </div>
                ))}
            </dl>
        </div>
    );
}

export default function HistoryDashboard() {
    const [wallet2, setWallet] = useState("Spot");
    const [currency, setCurrency] = useState("BNB");
    const [status, setStatus] = useState("Filled");
    const [selectedTrade, setSelectedTrade] = useState(null);
    const { tradeHistory, wallet} = useUserResources();
    const trades = tradeHistory;

    const hasTrades = trades.length > 0;

    return (
        <div className="min-h-screen w-full bg-[#EAE3D5] px-8 py-6">
            <div className="flex items-start gap-8">
                {/* Main dashboard */}
                <div className="flex-1">
                    {/* Header row: name + logo */}
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                            <Link
                                to="/user/dashboard"
                                className="p-1 rounded-md hover:bg-black/5 transition-colors text-gray-900"
                            >
                                <ArrowLeft size={22} strokeWidth={2} />
                            </Link>
                            <h1 className="font-serif text-2xl font-bold text-gray-900">
                                Wind Lê
                            </h1>
                        </div>
                        <WindyLogo />
                    </div>

                    {/* Amount / Locked */}
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-8 text-sm text-gray-900 mb-4">
                            <div className="flex items-center gap-2">
                                <Wallet size={16} strokeWidth={2} />
                                <span>
                  Amount: <span className="font-medium">{wallet?.available_balance}$ (USD)</span>
                </span>
                            </div>
                            <div className="flex items-center gap-2">
                                <Lock size={16} strokeWidth={2} />
                                <span>
                  Locked: <span className="font-medium">{wallet?.locked_balance}$ (USD)</span>
                </span>
                            </div>
                        </div>
                    </div>

                    <div className="border-b border-gray-700/60 mb-2" />

                    {/* Table */}
                    <table className="w-full text-sm">
                        <thead>
                        <tr className="text-gray-900 font-semibold">
                            <th className="text-left py-3 font-semibold w-12"></th>
                            <th className="text-left py-3 font-semibold">Coin</th>
                            <th className="text-left py-3 font-semibold">Status</th>
                            <th className="text-left py-3 font-semibold">Position</th>
                            <th className="text-left py-3 font-semibold">Amount</th>
                            <th className="text-left py-3 font-semibold">Time</th>
                        </tr>
                        </thead>
                        <tbody>
                        {hasTrades ? (
                            trades.map((trade) => (
                                <tr
                                    key={trade.id}
                                    onClick={() => setSelectedTrade(trade)}
                                    className="text-gray-800 cursor-pointer hover:bg-black/5 transition-colors"
                                >
                                    <td className="py-3">
                                        <div className="font-extrabold tracking-tight text-brand text-emerald-600 text-stroke-1 text-stroke-color-black text-xl">
                                            {trade.symbol}
                                        </div>
                                    </td>
                                    <td className="py-3">{trade.coin}</td>
                                    <td className="py-3">
                      <span className="flex items-center gap-2">
                        <span
                            className={`w-2.5 h-2.5 rounded-full ${trade.statusColor}`}
                        />
                          {trade.status}
                      </span>
                                    </td>
                                    <td className={`py-3 font-medium ${trade.positionColor}`}>
                                        {trade.position}
                                    </td>
                                    <td className="py-3">{trade.amount}</td>
                                    <td className="py-3">{trade.time}</td>
                                </tr>
                            ))
                        ) : (
                            <tr>
                                <td colSpan={6} className="py-10 text-center text-gray-500">
                                    You haven't made any trade yet
                                </td>
                            </tr>
                        )}
                        </tbody>
                    </table>
                </div>

                {/* Order Detail panel */}
                {selectedTrade && (
                    <OrderDetail
                        trade={selectedTrade}
                        onClose={() => setSelectedTrade(null)}
                    />
                )}
            </div>
        </div>
    );
}